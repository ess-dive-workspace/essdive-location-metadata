...
- Filename is named “locations.csv” or ends with *_locations.csv
...
- When publishing location metadata on ESS-DIVE, ensure you include the following:
    ...
    - Include the keyword **"ESS-DIVE Location Metadata Reporting Format"** within the dataset metadata Keywords field.
...
For a CRS with WGS84 datum, use the longitude / latitude fields in decimal degrees. N / E values are positive and S / W values are negative. Most common GPS-based apps use WGS84, i.e., coordinates from your cell phone are likely WGS84.

For all other CRS, all four alt_coordinate_* fields must be entered:
...
For vertical curves, like towers or wells, the representative point should be the position along the curve that is closest to the ground surface or water surface. Indicate otherwise in the notes field.

Coordinates that define the curve or surface polygon can also be optionally provided in one of two ways:
1. **Option 1:** Via the location_coordinates field following geojson format:
...
2. **Option 2:** Within a separate file of the following types. The filename must end in *_locations.{ext}
   - A. json: following the GeoJSON format RFC 7946. Note: per the specification only use for WGS84 horizontal datum and WGS 84 reference ellipsoid for elevation datum. The location_id must be in the id geojson field. Only geometry types Point, LineString, and Polygon are supported.
   - B. kml / kmz: the location_id must be in the name field of a placemark element. Only geometry types Point, LineString, and Polygon are supported.
  
## Location Description
A short description of the location should be provided in the location_description field. When possible, include terms like those in the table below.

<img width="642" height="493" alt="Screenshot 2026-05-28 at 4 32 28 PM" src="https://github.com/user-attachments/assets/6e349ae5-cf8f-4500-8170-e14e12f65e00"/>
